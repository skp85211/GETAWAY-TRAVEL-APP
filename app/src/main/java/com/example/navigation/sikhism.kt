...
import kotlinx.android.synthetic.main.fragment_sikhism.view.*
...
class sikhism : Fragment() {
    ...
    override fun onCreateView(
        ...
        val v=inflater.inflate(R.layout.fragment_sikhism, container, false)
        sikhismapi().getplaces().enqueue(object: Callback<List<place>> {
        ...
        f.setOnClickListener {
            var j = 0
            if (sikhismAdapter.checkedplaces.size > 0) {
                do {
                    var checked = sikhismAdapter.checkedplaces[j]
                    var sb = checked.name
                    val s = Sender(context, insertUrl, sb)
                    s.execute()
                    j++

                } while (j < sikhismAdapter.checkedplaces.size)
