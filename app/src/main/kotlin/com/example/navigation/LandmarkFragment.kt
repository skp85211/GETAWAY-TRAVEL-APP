...
class LandmarkFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val v=inflater.inflate(R.layout.fragment_landmark, container, false)
        landmarkapi().getplaces().enqueue(object: Callback<List<place>> {
        ...
        f.setOnClickListener {
            var j = 0
            if (landmarkAdapter.checkedplaces.size > 0) {
                do {
                    var checked = landmarkAdapter.checkedplaces[j]
                    var sb = checked.name
                    val s = Sender(context, insertUrl, sb)
                    s.execute()
                    j++

                } while (j < landmarkAdapter.checkedplaces.size)
    ...
    private fun places(places:List<place>)
    {
        ...
        recyclerViewPlaces.adapter=landmarkAdapter(places)
    }
}
